...
TEST(StepperWorker_test, active_past_target){
  ...
  int pin = 22;
  StepperWorkerFSM sw(1, as, pin, false);
  ...
  EXPECT_EQ(StepperWorkerFSMState::ACTIVE, sw.getState());
  ...
  EXPECT_EQ(StepperWorkerFSMState::PAST_TARGET, sw.getState());
}
...
TEST(StepperWorker_test, endstopTest){
  ...
  printf("time: %ld\n", elapsedtime);
  sw.loop(elapsedtime);
  EXPECT_EQ(StepperWorkerFSMState::ENDSTOP_HIT, sw.getState());
  ...
  EXPECT_EQ(StepperWorkerFSMState::ENDSTOP_WAITING, sw.getState());
...
TEST(StepperWorker_test, calibrationTest){
  ...
  printf("time: %ld\n", elapsedtime);
  EXPECT_EQ(StepperWorkerFSMState::INIT, sw.getState());

  sw.startCalibration();
  sw.loop(elapsedtime);
  EXPECT_EQ(StepperWorkerFSMState::CALIBRATING_UP, sw.getState());

  test_triggerEndStop(true);
  sw.loop(elapsedtime);
  EXPECT_EQ(StepperWorkerFSMState::CALIBRATING_ENDSTOPHIT, sw.getState());
  ...
  sw.loop(elapsedtime);
  printf("time: %ld\n", elapsedtime);
  EXPECT_EQ(StepperWorkerFSMState::CALIBRATION_FINISHED, sw.getState());
